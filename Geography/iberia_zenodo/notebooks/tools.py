...
class Geography(object):
    ...
    def plot_recension(self,fs_dict,gs_dict,df_dict,select_recension):
        ...
        if select_recension== 'Omega':
            ...
            co='dfTempX'
            p.circle(x='x', y='y',source=source,view=view, fill_color="red",size=6, fill_alpha=.9, line_color="red",line_alpha=0.6,legend="Localities in Xi",muted_alpha=0.2)
    ...
    def plot_recension_all(self,fs_dict,gs_dict,df_dict):
            ...
            TOOLTIPS = [("index", "$index"),("(x,y)", "($x, $y)")]
            p = figure(title=self.select_recension, width=1000, height=800, x_range=(1.5, 22), y_range=(35.5, 47),tools=tools,x_axis_location=None, y_axis_location=None,tooltips=TOOLTIPS)
            for i in self.fs_dict.values():
            ...
            source = ColumnDataSource(data=Locality)
            view = CDSView(source=source)
            p.circle(x='x', y='y',source=source,view=view, fill_color="dodgerblue",size=6,fill_alpha=.9,line_color="dodgerblue",line_alpha=0.6,legend="Localities in Omega",muted_alpha=0.2)
            for i in self.gs_dict.values():
            ...
            source = ColumnDataSource(data=Locality)
            view = CDSView(source=source)
            p.circle(x='x', y='y',source=source,view=view, fill_color="crimson",size=6, fill_alpha=.9, line_color="red",line_alpha=0.6,legend="Localities in Xi")
    ...
    def plot_compare_recension(self,fs_dict,gs_dict,df_dict):
            ...
            source = ColumnDataSource(data=Locality)
            view = CDSView(source=source)
            r.circle(x='x', y='y',source=source,view=view, size=5,fill_color='red', fill_alpha=.7,                         line_color='Crimson',line_alpha=0,legend="Localities in Xi")
            for i in gs_dict.values():
                            r.line(i[:,0],i[:,1], color='Crimson',legend="Boundary in Xi ",line_width=2)
            # Omega
            Locality={"x":list(self.df_dict["dfTemp"]['longitude']),"y":list(self.df_dict["dfTemp"]['latitude'])}
            source = ColumnDataSource(data=Locality)
            view = CDSView(source=source)
            r.circle(x='x', y='y',source=source,view=view, size=5,fill_color='blue', fill_alpha=0.8,   line_color='DodgerBlue',line_alpha=0,legend="Localities in Omega",muted_alpha=0.2)
            for i in fs_dict.values():
                r.line(i[:,0],i[:,1], color='DodgerBlue',legend="Boundary in Omega",line_width=2)
            r.circle(np.array(IbEq["longitude_Xi"]),np.array(IbEq["latitude_Xi"]), size=5.5,fill_color='green', fill_alpha=1, line_color='green',line_alpha=0.8,legend="Same Localities in Xi and Omega" )
...
def reformatCoord(row,longLat, xy='coord_x'):
    """this function extracts the integer and fraction part of the coordiantion in greek form each row in datafrmae
       for instance 
       >>>row=dfTemp.iloc[26]
       ID                                                                                       2.04.06.02
category                                                                                       city
...
def Js2Geodf_1(df): 
    ...
    for i, x in om.iterrows():
        ...
        dfout["type_sec"] = dfout.apply(lambda x: findTS_1(x["ID"], om), axis=1)
    return(dfout)
